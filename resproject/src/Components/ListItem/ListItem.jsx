...
const ListItem = ({ fetchURL }) => {
  ...
  return (
    <div className="flex h-fit py-[1.3vw] relative">
      {movies.map((item) => {
        return (
          <div className="w-[30vw] sm:w-[22vw] lg:w-[14.95vw]  flex-col p-[.14vw] cursor-pointer hover:scale-125 ease-in duration-500">
            <img
            ...
            />
            <div className="h-[8.4vw] w-[14.95vw] background flex items-center text-center rounded justify-center opacity-0 hover:opacity-100 absolute top-6 overflow-hidden hover:bg-black hover:top-[3.5px] hover:bg-opacity-30 ease-in duration-500">
              {item.title}
            </div>
          </div>
        );
