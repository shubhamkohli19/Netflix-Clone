...
const ListItem = ({ fetchURL }) => {
  ...
  return (
    <div className="flex h-fit py-5 relative">
      {movies.map((item, id) => {
        return (
          <div className="w-[227.5px] min-h-[125px] flex-col p-[3px] cursor-pointer hover:scale-125 ease-in duration-500">
            <img
            ...
            />
            <div className="h-[125px] w-[222px] background flex items-center text-center rounded justify-center opacity-0 hover:opacity-100 absolute top-6 overflow-hidden hover:bg-black hover:top-[3.5px] hover:bg-opacity-30 ease-in duration-500">
              {item.title}
            </div>
          </div>
        );
