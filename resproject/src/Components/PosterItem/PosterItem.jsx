...
const PosterItem = ({ fetchURL }) => {
  ...
  return (
    <div className="flex h-fit py-[1.3vw] relative">
      {movies.map((item) => {
        return (
          <a href={`https://www.themoviedb.org/movie/${item.id}`} key={item.id} className="w-[45vw] sm:w-[22vw] lg:w-[14.95vw] flex p-[.14vw] cursor-pointer hover:scale-110 ease-in duration-300 background">
            <img
